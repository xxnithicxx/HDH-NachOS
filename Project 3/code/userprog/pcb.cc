...
extern void StartProcessByID(int id);
...
{  
	...
	this->parentID = currentThread->processID;
	// Gọi thực thi Fork(StartProcessByID,id) => Ta cast thread thành kiểu int, sau đó khi xử ký hàm StartProcess ta cast Thread về đúng kiểu của nó.
 	this->thread->Fork(StartProcessByID,id);

	// Gọi mutex->V() để giải phóng miền găng.
	multex->V();
	// Trả về id.
	return id;
}
